        ...
        while(true){
            ...
            switch (opcao) {
                ...
                case 7:
                    ...
                    prompt.nextLine();
                    System.out.print("Digite as coordenadas x e y do vetor separadas por espaço: ");
                    String entrada2DP = prompt.nextLine();
                    String[] valores2DP = entrada2DP.trim().split("\\s+");

                    if (valores2DP.length != 2) {
                        System.out.println("Erro: Digite exatamente duas coordenadas!");
                        break;
                    }

                    try {
                        double x2D = Double.parseDouble(valores2DP[0]);
                        double y2D = Double.parseDouble(valores2DP[1]);

                        System.out.println("Escolha o eixo de projeção: ");
                        System.out.println("1 - Eixo X");
                        System.out.println("2 - Eixo Y");
                        int eixo = prompt.nextInt();
                        ...
                        double[] elementos2D = { x2D, y2D };
                        Vector vetor2D = new Vector(2, elementos2D);
                        Vector resultado2D = null;

                        switch (eixo) {
                            case 1:
                                resultado2D = Transformations.projection2DX(vetor2D);
                                break;
                            case 2:
                                resultado2D = Transformations.projection2DY(vetor2D);
                                break;

                            default:
                                System.out.println("Opção inválida de eixo! Use 1 ou 2.");
                                break;
                        }
                ...
                case 8:
                    ...
                    prompt.nextLine();
                    System.out.print("Digite as coordenadas x, y e z do vetor separadas por espaço: ");
                    String entrada3DP = prompt.nextLine();
                    String[] valores3DP = entrada3DP.trim().split("\\s+");

                    if (valores3DP.length != 3) {
                        System.out.println("Erro: Digite exatamente três coordenadas!");
                        break;
                    }

                    try {
                        double x3D = Double.parseDouble(valores3DP[0]);
                        double y3D = Double.parseDouble(valores3DP[1]);
                        double z3D = Double.parseDouble(valores3DP[2]);
                        ...
                        switch (eixo) {
                            case 1:
                                resultado3D = Transformations.projection3DX(vetor3D);
                                break;
                            case 2:
                                resultado3D = Transformations.projection3DY(vetor3D);
                                break;
                            case 3:
                                resultado3D = Transformations.projection3DZ(vetor3D);
                                break;
                            default:
                                System.out.println("Opção inválida de eixo! Use 1, 2 ou 3.");
                                break;
                ...
                case 9:
                    ...
                    prompt.nextLine();
                    System.out.print("Digite as coordenadas x e y do vetor separadas por espaço: ");
                    String entradaCis = prompt.nextLine();
                    String[] coordsCis = entradaCis.trim().split("\\s+");

                    if (coordsCis.length != 2) {
                        System.out.println("Erro: Digite exatamente duas coordenadas.");
                        break;
                    }

                    try {
                        double xCis = Double.parseDouble(coordsCis[0]);
                        double yCis = Double.parseDouble(coordsCis[1]);

                        System.out.print("Digite o valor de cisalhamento em X (kX): ");
                        double kX = Double.parseDouble(prompt.nextLine());

                        System.out.print("Digite o valor de cisalhamento em Y (kY): ");
                        double kY = Double.parseDouble(prompt.nextLine());

                        Vector vetorCis = new Vector(2, new double[]{xCis, yCis});
                        Vector resultadoCis = Transformations.cisalhamento2D(vetorCis, kX, kY);

                        System.out.println("\nVetor original:");
                        vetorCis.mostraVector();
                        System.out.println("\nVetor após cisalhamento:");
                        resultadoCis.mostraVector();
                    } catch (NumberFormatException e) {
                        System.out.println("Erro: Entrada inválida.");
                    }
                    break;
